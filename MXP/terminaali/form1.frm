...
For f = 9 To Len(startdir$)
If Mid$(startdir$, f, 1) = "\" Then
   useri = Mid$(startdir$, 9, f - 9) + "@"
   Exit For
End If
Next
If startdir$ <> "" Then
   If ReadINI("Aja", "start", "", startdir$ + "\aja.ini") = "false" Or ReadINI("Aja", "start", "", startdir$ + "\aja.ini") = "" Then
         WritePrivateProfileString "Aja", "exe", "start.exe /unix /usr/bin/xfce4-terminal " + Command$, startdir$ + "\aja.ini"
