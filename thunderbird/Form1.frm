...
Sub aktivoilinux()
    ...
    keybd_event VK_STARTKEY, 0, KEYEVENTF_KEYUP, 0
         ...
         WritePrivateProfileString "Aja", "exe", "start.exe /unix /usr/bin/xdg-email " + c$, startdir$ + "\aja.ini"
         ...
         WritePrivateProfileString "Aja", "aktivoi", "thunderbird", startdir$ + "\aja.ini"
...
If startdir$ <> "" Then
   If ReadINI("Aja", "start", "", startdir$ + "\aja.ini") = "false" Or ReadINI("Aja", "start", "", startdir$ + "\aja.ini") = "" Then
         WritePrivateProfileString "Aja", "exe", "/usr/bin/xdg-email " + c$, startdir$ + "\aja.ini"
         WritePrivateProfileString "Aja", "path", "c:\windows\command", startdir$ + "\aja.ini"
         WritePrivateProfileString "Aja", "start", "true", startdir$ + "\aja.ini"
         WritePrivateProfileString "Aja", "system", "unix", startdir$ + "\aja.ini"
         If c$ <> "" Then
         ...
         Else
            WritePrivateProfileString "Aja", "aktivoi", "thunderbird", startdir$ + "\aja.ini"
            If ReadINI("Thunderbird", "Lock", "", startdir$ + "\aja.ini") <> "True" Then
               WritePrivateProfileString "Thunderbird", "Lock", "True", startdir$ + "\aja.ini"
             Else
               End
            End If
...
If c$ = "" Then
  vastaus = MsgBox("Do you want to remove the dock from the panel?", vbYesNo + vbQuestion, "Poisto")
